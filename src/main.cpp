...
// Copyright (c) 2014 The Futurecoin developers
...
int CMerkleTx::GetBlocksToMaturity() const
{
    ...
    if(GetHeightInMainChain() <= 400) {
        maturity = 100;
    }
    else if(GetHeightInMainChain() <= 1401) {
        maturity = 10080; //1 week
    }
    else if(GetHeightInMainChain() <= 14001) {
        maturity = 10080 * 2; //2 weeks
    }
    else if(GetHeightInMainChain() <= 140001) {
        maturity = 10080 * 3; //3 weeks
    }
    else if(GetHeightInMainChain() > 140001) {
        maturity = 10080 * 4; //4 weeks
    }
    else {
        maturity = 100; //should never be reached
    }

    return max(0, (maturity+20) - GetDepthInMainChain());
}
...
static const int64 nStartSubsidy = 500 * COIN;
...
static const int64 nTargetTimespan = 180; // 3 minutes (NUM_ALGOS * 60 seconds)
static const int64 nTargetSpacing = 180; // 3 minutes (NUM_ALGOS * 60 seconds)
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FuturecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FuturecoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("Futurecoin miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    ...
    {
        printf("Futurecoin miner terminated\n");
        throw;
    }
}
